#!/usr/bin/env python3
"""
Dungeon Mastron Animation Generator (Vertical 720x1280)
Generates Ken Burns style MP4 animations at 720x1280 using FFmpeg.

Usage: place alongside animation_manifest.json and run:
  python3 animation_generator_vertical.py
"""
...
OUTPUT_W = 720
OUTPUT_H = 1280
_SCALE_FACTOR = INTERNAL_W / OUTPUT_W

# Pixel offsets (total over the full clip), expressed in INTERNAL pixels.
...
def generate_video(image_path, output_path, effect_key):
    ...
    cmd = [
        ...
        '-loop', '1',
        '-i', str(image_path),
        '-vf', f"scale={INTERNAL_W}:{INTERNAL_H}:flags=lanczos,zoompan={effect['zoompan']}:s={INTERNAL_W}x{INTERNAL_H}:fps={FPS},scale={OUTPUT_W}:{OUTPUT_H}:flags=lanczos",
        '-r', str(FPS),
        '-vsync', 'cfr',
        '-c:v', 'libx264',
        '-tune', 'stillimage',
...
def main():
    ...
    animations_dir = Path('images/animations_vertical')
